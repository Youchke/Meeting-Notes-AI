...
if __name__ == "__main__":
    if os.environ.get("GEMINI_API_KEY"):
        ...
        try:
            GEMINI_API_KEY = open("gemini_apikey.txt", "r").read().strip()
        except FileNotFoundError:
            # Fallback to check for OPENAI_API_KEY just in case user put it there or .env
            # But better to be strict for the migration.
            print("Error: gemini_apikey.txt not found and GEMINI_API_KEY environment variable not set.")
            print("Please create a 'gemini_apikey.txt' file with your Google Gemini API key.")
            exit(1)

    print("Welcome to the meeting summarizer (Powered by Gemini)!")
